            ...
            <div class="sidebar">
                ...
                <nav class="mt-2">
                    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                        ...
                        <li class="nav-item">
                            <a href="<?= base_url('admin/keluar') ?>" class="nav-link active">
            ...
            <section class="content">
                <div class="container-fluid">
                    <div class="alert" style="margin-top: 70px;" role="alert">
                        <h3 class="m-0"><i class="fa-solid fa-person-hiking"></i> Data Pindah Keluar</h3>
                    </div>
                                ...
                                <tbody>
                                    ...
                                    foreach ($keluar as $klr) { ?>
                                        <tr>
                                            ...
                                            <td>
                                                <form action="<?= base_url() ?>/admin/hapus_keluar/<?= $klr['id_keluar'] ?>" method="post">
                                                    <button type="button" class="badge badge-warning" style="border: none; margin-right: 10px;" data-toggle="modal" data-target="#edit_modal<?= $klr['id_keluar'] ?>">
                                        ...
                                        <div class="modal fade" id="edit_modal<?= $klr['id_keluar'] ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title" id="myModalLabel">Edit Pindah Keluar</h4>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <form method="POST" action="<?= base_url('admin/edit_keluar/' . $klr['id_keluar']) ?>">
                                                            <?= csrf_field(); ?>
                                                            <div class="form-group">
                                                                <label>Nama</label>
                                                                <input type="text" name="nama" value="<?= $klr['nama'] ?>" class="form-control" required>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Nik</label>
                                                                <input type="text" name="nik" value="<?= $klr['nik'] ?>" class="form-control" required>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>No KK</label>
                                                                <input type="text" name="no_kk" value="<?= $klr['no_kk'] ?>" class="form-control" required>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Tanggal Lahir</label>
                                                                <input type="date" name="tanggal" value="<?= $klr['tanggal'] ?>" class="form-control" required>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Alamat</label>
                                                                <select name="alamat" class="form-control" required>
                                                                    <option>-- Pilih --</option>
                                                                    <option value="Ulurea" <?= $klr['alamat'] == "Ulurea" ? 'selected' : "" ?>>Ulurea</option>
                                                                    <option value="Pakkalolo" <?= $klr['alamat'] == "Pakkalolo" ? 'selected' : "" ?>>Pakkalolo</option>
                                                                    <option value="Karo" <?= $klr['alamat'] == "Karo" ? 'selected' : "" ?>>Karo</option>
                                                                    <option value="Boting" <?= $klr['alamat'] == "Boting" ? 'selected' : "" ?>>Boting</option>
                                                                    <option value="Lengkong" <?= $klr['alamat'] == "Lengkong" ? 'selected' : "" ?>>Lengkong</option>
                                                                </select>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Jenis Kelamin</label><br>
                                                                <input type="radio" name="jenis_kelamin" required value="Laki-laki" <?= $klr['jenis_kelamin'] == "Laki-laki" ? 'checked' : "" ?>> Laki-laki<br>
                                                                <input type="radio" name="jenis_kelamin" required value="Perempuan" <?= $klr['jenis_kelamin'] == "Perempuan" ? 'checked' : "" ?>> Perempuan<br>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Tanggal Pindah Keluar</label>
                                                                <input type="date" name="tanggal_keluar" value="<?= $klr['tanggal_keluar'] ?>" class="form-control" required>
                                                            </div>
