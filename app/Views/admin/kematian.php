            ...
            <div class="sidebar">
                ...
                <nav class="mt-2">
                    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                        ...
                        <li class="nav-item">
                            <a href="<?= base_url('admin/kematian') ?>" class="nav-link active">
            ...
            <section class="content">
                <div class="container-fluid">
                    <div class="alert" style="margin-top: 70px;" role="alert">
                        <h3 class="m-0"><i class="fa-solid fa-book-skull"></i> Data Kematian</h3>
                    </div>

                    <div class="card" style="margin-bottom: 70px;">
                        <div class="card-header">
                            <h5 class="card-title m-0">Tabel Data Kematian</h5>
                                ...
                                <tbody>
                                    ...
                                    foreach ($kematian as $kmt) { ?>
                                        <tr>
                                            ...
                                            <td><?= $kmt['tanggal'] ?></td>
                                            <td><?= $kmt['alamat'] ?></td>
                                            <td><?= $kmt['jenis_kelamin'] ?></td>
                                            <td><?= $kmt['tanggal_input'] ?></td>
                                            <td><?= $kmt['tanggal_kematian'] ?></td>
                                            <td>
                                                <form action="<?= base_url() ?>/admin/hapus_kematian/<?= $kmt['id_kematian'] ?>" method="post">
                                                    <button type="button" class="badge badge-warning" style="border: none; margin-right: 10px;" data-toggle="modal" data-target="#edit_modal<?= $kmt['id_kematian'] ?>">
                                        ...
                                        <div class="modal fade" id="edit_modal<?= $kmt['id_kematian'] ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <form method="POST" action="<?= base_url('admin/edit_kematian/' . $kmt['id_kematian']) ?>">
                                                            <?= csrf_field(); ?>
                                                            <div class="form-group">
                                                                <label>Nama</label>
                                                                <input type="text" name="nama" value="<?= $kmt['nama'] ?>" class="form-control" required>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Nik</label>
                                                                <input type="text" name="nik" value="<?= $kmt['nik'] ?>" class="form-control" required>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Tanggal Lahir</label>
                                                                <input type="date" name="tanggal" value="<?= $kmt['tanggal'] ?>" class="form-control" required>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Alamat</label>
                                                                <select name="alamat" class="form-control" required>
                                                                    <option>-- Pilih --</option>
                                                                    <option value="Ulurea" <?= $kmt['alamat'] == "Ulurea" ? 'selected' : "" ?>>Ulurea</option>
                                                                    <option value="Pakkalolo" <?= $kmt['alamat'] == "Pakkalolo" ? 'selected' : "" ?>>Pakkalolo</option>
                                                                    <option value="Karo" <?= $kmt['alamat'] == "Karo" ? 'selected' : "" ?>>Karo</option>
                                                                    <option value="Boting" <?= $kmt['alamat'] == "Boting" ? 'selected' : "" ?>>Boting</option>
                                                                    <option value="Lengkong" <?= $kmt['alamat'] == "Lengkong" ? 'selected' : "" ?>>Lengkong</option>
                                                                </select>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Jenis Kelamin</label><br>
                                                                <input type="radio" name="jenis_kelamin" required value="Laki-laki" <?= $kmt['jenis_kelamin'] == "Laki-laki" ? 'checked' : "" ?>> Laki-laki<br>
                                                                <input type="radio" name="jenis_kelamin" required value="Perempuan" <?= $kmt['jenis_kelamin'] == "Perempuan" ? 'checked' : "" ?>> Perempuan<br>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Tanggal Kematian</label>
                                                                <input type="date" name="tanggal_kematian" value="<?= $kmt['tanggal_kematian'] ?>" class="form-control" required>
                                                            </div>
